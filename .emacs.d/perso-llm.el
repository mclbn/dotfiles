;;; perso-llm.el --- Local LLM stack management -*- lexical-binding: t; -*-

;; Portainer-backed start/stop/status for the local model stack, plus the
;; top-level LLM menu.  The prompt builder that used to live here is now the
;; `gptel-builder' package.
...
(declare-function gptel-builder "gptel-builder")
...
(defun perso/llm-menu--select-backend ()
  "Select a gptel backend, updating `gptel-backend' and `gptel-model'."
  (interactive)
...
(transient-define-prefix perso/llm-menu ()
  ...
  ["LLM Stack"
   ...
   (:info "")
   ("b" perso/llm-menu--select-backend :description perso/llm-menu--backend-desc :transient t)
   ("m" perso/llm-menu--select-model :description perso/llm-menu--model-desc :transient t)
   ("s" perso/llm-menu--activate-stack :description perso/llm-menu--stack-desc :transient t)]
  [["Gptel"
